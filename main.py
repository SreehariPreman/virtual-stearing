...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    if len(co) == 2:
        ...
        cv2.circle(img=image, center=(int(xm), int(ym)), radius=radius, color=(0, 255, 0), thickness=15)

        l = (int(math.sqrt((co[0][0] - co[1][0]) ** 2 * (co[0][1] - co[1][1]) ** 2)) - 150) // 2
        cv2.line(image, (int(xa), int(ya)), (int(xb), int(yb)), (0, 255, 0), 20)
        if co[0][0] > co[1][0] and co[0][1]>co[1][1] and co[0][1] - co[1][1] > 65:
            ...
            cv2.line(image, (int(xbp), int(ybp)), (int(xm), int(ym)), (0, 255, 0), 20)
    ...
    cv2.imshow('webcam preview', cv2.flip(image, 1))
